...
Returns an existing _**Rhino Test Case**_ collection.
...
> Scenarios are seperated by an empty line, followed by `>>>` followed by another empty line.
...
Add additional _**Rhino Test Cases**_ into an existing collection. If the _**Test Case ID**_.

> Please note, there is no duplication check here, and you can add tests with the same ID.
> However, Rhino Engine, will not execute them, since it will distince all IDs before running automation.
